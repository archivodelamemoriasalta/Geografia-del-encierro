...
function mostrarDepartamento(depto) {
    ...
    let filtradas = personas.filter(p => normalizar(p.departamento) === deptoNorm);

    tituloPanel.textContent = depto;
    let html = `<h3>📍 ${filtradas.length} personas en ${depto}</h3>`;

    if (deptoNorm === "CAPITAL") {
        const normales = filtradas.filter(p => !p.observaciones.toLowerCase().includes("policia federal"));
        const federales = filtradas.filter(p => p.observaciones.toLowerCase().includes("policia federal"));
        ...
        html += `
            ...
            <h4 style="color:#ffaa00; margin:20px 0 8px;">🚔 Provenientes de Policía Federal (${federales.length})</h4>
...
async function cargarTodo() {
    ...
    filas.slice(1).forEach(f => {
        ...
        personas.push({
            ...
            departamento: depto,
            observaciones: (c[11] || "").replace(/null/g, "") || "Sin observaciones"
        });
